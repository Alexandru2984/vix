...
APP_PORT="${APP_PORT:-$("${ROOT_DIR}/scripts/find_free_port.sh" 18280 "${APP_HOST}")}"
...
for _ in {1..80}; do
  ...
  if ! kill -0 "${PID}" >/dev/null 2>&1; then
    echo "server exited during load startup" >&2
...
BASE_URL="http://${APP_HOST}:${APP_PORT}" npm run test:load

echo "load ok: websocket load check passed on http://${APP_HOST}:${APP_PORT}"
